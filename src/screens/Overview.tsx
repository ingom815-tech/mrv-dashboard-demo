...
function ChartTooltip({
  ...
  return (
    <div className="rounded-xl border border-line bg-white px-3.5 py-3 text-[12px] shadow-card">
    ...
    <div className="flex h-screen min-h-0 flex-col gap-4 px-7 py-5">
      ...
      <header className="flex shrink-0 items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-2 pb-1">
          <div className="flex items-center gap-1.5 rounded-lg border border-line bg-white px-3 py-1.5 text-[12px] text-slate-500">
          ...
          <button
            ...
            className="rounded-lg bg-accent px-3.5 py-1.5 text-[12px] font-semibold text-white transition-opacity hover:opacity-90"
          >
            산정근거 보기
          </button>
        </div>
      ...
      <section className="grid shrink-0 grid-cols-3 gap-4" aria-label="핵심 성과">
        ...
        >
          ...
          <div className="mt-2 text-[12px] text-slate-400">전력 절감분 환산 · 냉매 배출은 별도 관리</div>
        </button>
        <button
          onClick={() => setMenu("verify")}
          className="rounded-2xl border border-line bg-white p-5 text-left shadow-card transition-colors hover:border-accent/40"
        >
          <div className="text-[13px] font-medium text-slate-500">검증 상태</div>
          <div className="mt-1.5 flex items-baseline gap-2">
            <span className="text-[32px] leading-none font-bold text-review">{k.verifyState}</span>
          </div>
          <div className="mt-2 text-[12px] text-slate-400">
            데이터 신뢰도 <span className="font-semibold text-navy">{fmt(k.trustRate * 100, 1)}%</span>{" "}
            · 검토 필요 <span className="font-semibold text-review">{k.reviewCount}건</span>
          </div>
        </button>
      </section>
        ...
        <div className="flex min-h-0 flex-col rounded-2xl border border-line bg-white p-5 shadow-card">
          <div className="flex shrink-0 items-center justify-between">
            ...
            <div className="flex items-center gap-4 text-[12px] text-slate-500">
              ...
              <span className="flex items-center gap-1.5">
                <span className="inline-block h-2.5 w-4 rounded-sm bg-teal/20" aria-hidden />
                절감량
              </span>
              <span className="text-slate-400">단위 MWh/월</span>
            </div>
          </div>
          ...
          <div className="shrink-0 pt-2 text-[12px] text-slate-400">
            {sel ? (
              <span>
                ...
                <button onClick={() => setMenu("verify")} className="ml-2 font-medium text-accent hover:underline">
                  상세 보기
                </button>
              </span>
            ) : (
              <span>월을 클릭하면 해당 월의 데이터 이슈와 제외 내역을 확인할 수 있습니다.</span>
            )}
          </div>
        </div>

        {/* 신뢰도 요약 — 상태 3줄 + 산정근거 버튼 (v2.1 §2.1) */}
        <div className="flex min-h-0 flex-col rounded-2xl border border-line bg-white p-5 shadow-card">
          ...
          <div className="mt-4 space-y-4">
            ...
            <div className="flex items-start gap-2.5">
              ...
              <div>
                ...
                <div className="mt-0.5 text-[12px] leading-relaxed text-slate-400">
                  적합도 기준 {mrv.baseline.pass ? "충족" : "미충족"} · 상세는 산정근거
                </div>
              </div>
            </div>
            <div className="flex items-start gap-2.5">
              <span
                className="mt-0.5 shrink-0 text-[13px] leading-none font-bold text-review"
                aria-hidden
              >
                △
              </span>
              <div>
                <div className="text-[13px] font-semibold text-navy">검토 필요 {k.reviewCount}건</div>
                {topReview && (
                  <div className="mt-0.5 text-[12px] leading-relaxed text-slate-400">
          ...
          <div className="mt-auto space-y-2 pt-4">
            ...
            <div className="flex justify-center gap-3 text-[12px]">
              <button onClick={() => setMenu("verify")} className="text-accent hover:underline">
                데이터 이슈 ›
              </button>
              <button onClick={() => setMenu("analysis")} className="text-accent hover:underline">
                설비 원인 ›
              </button>
            </div>
          </div>
